 ...
 * @return void
 */
function illume_register_block_patterns()
{
    $block_pattern_categories = array(
        'illume' => array('label' => esc_html__('illume', 'illume')),
        'pages' => array('label' => esc_html__('Pages', 'illume')),
    );
     ...
     * }
     */
    $block_pattern_categories = apply_filters('illume_block_pattern_categories', $block_pattern_categories);
        ...
        if (is_file($file_path) && pathinfo($file_path, PATHINFO_EXTENSION) == 'php') {
            ...
            if ($file !== 'pricing-table-2.php') {
                ...
                if (!empty($pattern_content)) {
                    ...
                    $pattern_name = 'illume-pattern-' . sanitize_title($pattern_title); // Generate a unique name for the pattern

                    register_block_pattern(
                        $pattern_name,
                        array(
                            'title' => sprintf(__('%s', 'illume'), $pattern_title),
                            'content' => $pattern_content,
                            'categories' => array('illume'), // Category under which your pattern should appear
                        )
                    );
                }
            }
...
add_action('init', 'illume_register_block_patterns', 9);
